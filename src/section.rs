...
impl Section {
  ...
  pub fn write_to<'s>(&self, mut buffer: SliceVec<'s, u8>) {
    for line in self.lines.iter() {
      match line {
        SectionLine::Label(_) => (),
        SectionLine::JumpRelativeLabel(_, _) => {
          panic!("tried to write a section with unresolved labels")
        }
        SectionLine::JumpRelativeDelta(i, Condition::Always) => {
          buffer.push(0x18);
          buffer.push(*i as u8);
        }
        SectionLine::JumpRelativeDelta(i, cond) => {
          buffer.push(((*cond as u8) + 4) << 3);
          buffer.push(*i as u8);
        }
        SectionLine::Nop => {
          buffer.push(0);
